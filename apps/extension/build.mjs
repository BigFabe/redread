...
for(const browser of ['chrome','firefox']){
  ...
  await build({entryPoints:['background','popup'].map(name=>join(root,'src',`${name}.ts`)),bundle:true,outdir,format:'iife',target:['chrome120','firefox142'],legalComments:'eof'});
  ...
  // Reuse the rounded app icon so toolbar icons keep the same corner radius.
  for(const size of [16,32,48,128])await sharp(await readFile(resolve(root,'../web/app/icon.png'))).resize(size,size).png().toFile(join(outdir,`icons/${size}.png`));
