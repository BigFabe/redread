...
test('named voices validate, trim and reject duplicate names',()=>{
  assert.deepEqual(customVoices([{name:' Narrator ',voice:' id '}]),[{name:'Narrator',voice:'id'}]);
...
test('named voices load from env and reset temporary settings; selected voice survives settings reset',()=>{
  ...
  const contents='TTS_CUSTOM_VOICES=\'[{"name":"Narrator","voice":"env-id"}]\'\n';writeFileSync(file,contents);
  try{
    execFileSync(process.execPath,['--import','tsx','--input-type=module','-e',`
      ...
      assert.deepEqual(settings().customVoices,[{name:'Narrator',voice:'env-id'}]);
      saveSettings({...settings(),llmModel:'test',ttsModel:'test',customVoices:[{name:'Temporary',voice:'temp-id'}]});
