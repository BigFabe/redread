...
- `TTS_CUSTOM_VOICES`: benannte Stimmen als JSON-Liste, z. B. `'[{"name":"Erzähler","voice":"reference-id"}]'`. Unter „Eigene Stimmen“ als Tabelle editierbar; Webapp und Erweiterungs-Popup bieten sie vor dem Start zur Auswahl an. Die pro Artikel gespeicherte Auswahl hat Vorrang vor Sprachzuordnung und Standardstimme, auch beim Fortsetzen. Ohne Auswahl bleibt das bisherige Verhalten. IDs müssen zum aktiven TTS-Anbieter passen. Wie alle Webapp-Einstellungen gilt die Liste nur bis zum Worker-Neustart; für dauerhafte Einträge `.env` nutzen.
